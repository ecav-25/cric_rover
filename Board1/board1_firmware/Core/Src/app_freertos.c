	...
	*/
	...
	void readSensorsTask(void *argument)
	{
	  /* USER CODE BEGIN readSensorsTask */
		static uint8_t temp_comm_fail_count = 0;
		static uint8_t batt_comm_fail_count = 0;
		...
		for(;;){
			...
			if (temp_st == TEMP_ERR) {
				...
				if (temp_comm_fail_count >= 5) {
					safety_stop_and_halt();
				}
			}
			else {
				temp_comm_fail_count = 0;
			}
			...
			if (batt_st == BATT_ERR) {
				...
				if (batt_comm_fail_count >= 5) {
					safety_stop_and_halt();
				}
			}
			else {
				batt_comm_fail_count = 0;
